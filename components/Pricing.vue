    ...
    <section id="pricing_sect" class="pb-40">
        <div class="container mx-auto px-4">
            <div class="title text-center pb-20">
                <span v-gsap.whenVisible.from="{ opacity: 0, y: -32 }" class="bg-btn font-inter text-white text-base font-normal px-4 py-2 rounded-4xl inline-block mb-3">Pricing</span>
                <h2 v-gsap.whenVisible.from="{ opacity: 0, y: -32 }" class="text-white font-jost font-bold text-4xl"> Our Pricing </h2>
                <p v-gsap.whenVisible.from="{ opacity: 0, y: -32 }" class="text-white font-jost font-normal text-xl max-w-2/3 mx-auto pt-4">Projectile helps you collaborate more smoothly and communicate better.  Projectile helps you collaborate 
            ...
            <div class="pricing_box flex justify-between xl:justify-center flex-wrap sm:pt-10">
                <div v-gsap.whenVisible.from="{ opacity: 0, y: 100 }" v-for="(item,index) in items" :class="['prc_card px-4 sm:px-10 md:px-3 mb-10 md:mb-0 lg:px-6 xl:px-10 2xl:px-14 py-10 md:py-6 xl:py-16 rounded-lg border-white/10 w-full sm:w-[100%%] md:w-[30%] lg:w-[30%]',{'md:scale-[1.1] bg-btn':item.reqomendet,'bg-white/10':!item.reqomendet}]">
                    ...
                    <div class="price py-4 xl:py-10">
                        <h2 class="font-jost md:text-2xl xl:text-5xl text-white font-bold xl:pb-3"><span class="relative top-0 xl:-top-6 text-lg">$</span>  {{ item.price  }} USD</h2>
                        <p  class="font-jost pt-1 font-thin text-base  md:text-xs lg:text-base text-white"> {{ item.price_info  }}</p>
                    </div>
                    <ul  class="space-y-2">
