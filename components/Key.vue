<template>
    <section id="key_aspect_sect" class="bg-btn/5 py-32">
      <div class="container mx-auto sm:px-4">
        ...
        <div class="key_box flex justify-between flex-wrap gap-10">
          <div
            v-for="(item, index) in items"
            :key="index"
            :class="[
              'key_item text-center max-w-[360px] mb-5 p-5',
              { 'flex justify-center items-center bg-btn/10 rounded-3xl min-w-[360px]': item.isSpecial },
            ]"
          >
            ...
            <div class="cntn">
              <h4 class="font-inter text-xl text-white font-semibold pt-8 pb-4">{{ item.title }}</h4>
              <p v-if="!item.isSpecial" class="font-inter text-base text-white font-normal">{{ item.description }}</p>
              <a
                v-if="item.isSpecial"
                href="#"
                class="font-jost inline-block mt-10 text-base bg-btn border-2 border-white/30 rounded-4xl text-white font-normal px-10 py-4"
              >
                {{ item.buttonText }}
              </a>
            </div>
  ...
  const items = ref([
    {
      ...
      description: "Increase online store conversion up to 25% with our online visual merchandising interface.",
    },
    {
      icon: iconImage,
    ...
    {
      icon: iconImage,
      title: "Ask & Collect reviews from customers",
      description: "Increase online store conversion up to 25% with our online visual merchandising interface.",
    },
    {
      icon: iconImage,
      title: "Access your business from any device",
      description: "Increase online store conversion up to 25% with our online visual merchandising interface.",
    },
    {
      icon: iconImage,
      title: "Schedule your bookings online",
      description: "Increase online store conversion up to 25% with our online visual merchandising interface.",
    },
    {
      icon: iconImage,
      title: "Contact your customers from one inbox",
      description: "Increase online store conversion up to 25% with our online visual merchandising interface.",
    },
    {
      isSpecial: true,  
